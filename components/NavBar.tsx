...
import { Moon, Baby, User, WifiOff } from 'lucide-react';
...
export const NavBar: React.FC = () => {
  const { mode, isKid, toggleMode } = useApp();
  const isOnline = useOnlineStatus();
  
  return (
    <nav className={`sticky top-0 z-30 shadow-sm flex flex-col transition-colors ${isKid ? 'bg-white border-b-4 border-kid-primary' : 'bg-white/80 backdrop-blur border-b border-slate-200'}`}>
      ...
      {!isOnline && (
        <div className={`w-full py-2 px-4 flex items-center justify-center gap-2 text-sm font-bold animate-in slide-in-from-top-2 ${isKid ? 'bg-amber-400 text-amber-900' : 'bg-slate-800 text-white'}`} role="alert">
          <WifiOff size={16} />
          <span>
